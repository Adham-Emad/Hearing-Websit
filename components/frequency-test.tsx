...
// Note: We are using the literal value 50 instead of AUDIOMETRY_STARTING_DB
import { dbToGain, AUDIOMETRY_STARTING_DB } from "@/lib/audio-constants"

interface FrequencyTestProps {
  ear: "left" | "right"
  // The threshold is now correctly stored as the dB value
...
  // 1. CHANGE: Start volume at 50 dB
...
  
  // Calculate linear gain from the dB state
  const currentGain = dbToGain(currentDb)
...
    setIsPlaying(true)
    // Use the calculated gain based on the dB state
    toneGeneratorRef.current.startLoopingTone(currentFrequency, currentGain, ear)
  }

  // SLIDER CHANGE: Slider value is now dB
  const handleVolumeChange = (newVolumeDb: number[]) => {
    const newDb = newVolumeDb[0]
    setCurrentDb(newDb)
    if (toneGeneratorRef.current && isPlaying) {
      // Convert dB back to gain for the tone generator
      toneGeneratorRef.current.setVolume(dbToGain(newDb))
    }
  }

  // BUTTONS CHANGE: Fixed 5 dB step (consistent and responsive change)
  const handleVolumeDown = () => {
    // FIXED DB STEP: Decrease by 5 dB
    const newDb = Math.max(0, currentDb - 5)
    setCurrentDb(newDb)
    if (toneGeneratorRef.current && isPlaying) {
      toneGeneratorRef.current.setVolume(dbToGain(newDb))
    }
...
    // FIXED DB STEP: Increase by 5 dB
    const newDb = Math.min(100, currentDb + 5)
    setCurrentDb(newDb)
    if (toneGeneratorRef.current && isPlaying) {
      toneGeneratorRef.current.setVolume(dbToGain(newDb))
    }
...
    // RESULTS CHANGE: Store the actual dB value (the threshold)
    const newResults = [...results, { frequency: currentFrequency, threshold: currentDb }]
    setResults(newResults)

    if (currentFrequencyIndex < testFrequencies.length - 1) {
      setCurrentFrequencyIndex(currentFrequencyIndex + 1)
      // 2. CHANGE: Reset volume to 50 dB for next frequency
      setCurrentDb(50)
    } else {
...
              <Slider
                value={[currentDb]} // Use the dB state directly
                onValueChange={handleVolumeChange}
                max={100} // Set max to 100 dB
                min={0} // Set min to 0 dB
                step={5} // Set step to a linear 5 dB
                className="w-64"
              />
